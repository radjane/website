  ...
  <div class="w-md  dark:bg-gray-800">
    <div class="md:container md:w-auto  dark:bg-gray-800 pb-1 pt-20 mx-1">
      ...
      <div class="transition duration-500 ease-in-out   
        ...
        <!-- 作者 的首个字符 -->
        <avatar :name="item.author" :background-color="extractColorByName(item.author)" color="#fff" :inline="true"
          class="w-12 h-12 rounded-full float-right mt-8" />
        <p @click="newsDetails(item.id)" class="cursor-pointer text-lg h-8 p-1 text-left  
          font-bold
          hover:underline md
        ...
        <p
          ...
          {{ item.pubDate }}.点击次数:{{ item.readCount }}.
        </p>
        <p
          class="text-xs p-1 text-left 
          text-gray-600 
        ...
        <p>
          <span
            class="bg-blue-100 text-blue-800 text-xs font-semibold mr-2 px-2.5 py-0.5 rounded dark:bg-blue-200 dark:text-blue-800">开源资讯</span>
        </p>
      </div>
      <!-- 滚动刷新 -->
...
export default defineComponent({
  ...
  setup() {
    ...
    const newsList = () => {
      OscNews.newsList(param).then((res) => {
